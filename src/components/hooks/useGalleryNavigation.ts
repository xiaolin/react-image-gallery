...
export function useGalleryNavigation({
  ...
  const slideToIndexWithStyleReset = useCallback(
    (nextIndex: number, event?: React.SyntheticEvent | Event): void => {
  ...
  const getContainerStyle = useCallback(
    ...
    }: GetContainerStyleOptions = {}): CSSProperties => {
   ...
   */
  const getExtendedSlides = useCallback((): ExtendedSlidesResult => {
    if (!infinite || totalSlides <= 1) {
      return {
        extendedItems: items,
        getSlideKey: (index: number) => `slide-${index}`,
        getRealIndex: (displayIdx: number) => displayIdx,
      };
    }
    ...
    return {
      ...
      getRealIndex: (displayIdx: number) => {
